...
export class CollegeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.selectedCollege = this.collegeData.find(
      (college) => college["abbreviation"] === this.collegeAbbreviation
    );
  ...
  initGraph(): void {
    ...
    const y = d3
      ...
      .domain(majorArray.map((entry: { major: any }) => entry.major)) // domain should be an array of all the majors in this collge
    ...
    const x = d3
      ...
      .domain([0, d3.max(majorArray, (d: { total: any }) => d.total)]); // domain of the bar chart from 0 to the biggest total in one major
    ...
    const xAxis = d3
      ...
      .tickSize(4)
      .ticks(1)
      .tickValues([0, d3.max(majorArray, (d: { total: any }) => d.total)]);
    ...
    let rects = bargroups
      ...
      .enter()
      .append("rect")
      .attr("y", (d: { data: { major: any } }) => y(d.data.major)) // this d is not the same as the d above, sample: {0: 0, 1: 286, data: the actual original data before stacked}
      ...
      .on("mouseout", (_d: any, _i: any, _n: any) => {
        tip.hide();
        d3.selectAll("rect")
          .transition()
          .duration(100)
      ...
      .on(
        "click",
        (d: {
          data: { major: string; majorCode: { toString: () => string } };
        }) => {
      ...
      .html((d) => {
        ...
        if (d.data.majorCode == 93 && d[0] == 18) {
          key = Object.keys(obj).find((key) => {
            return (
              obj[key] === d2 && key !== "total" && key !== "Asian American"
            );
          });
        } else {
          key = Object.keys(obj).find(
        ...
        return ` 
          ...
          obj.major
        } ${this.level !== "nondegree" ? obj.degree : ""}</span></h6>
